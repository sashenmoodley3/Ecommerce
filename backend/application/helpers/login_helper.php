...
if ( ! function_exists('_is_user_login'))
{
     function _is_user_login($thi)
    {
        ...
        $unique_key = _get_current_user_key($thi);
        if(!empty($userid)){
            $data       =  $thi->db->query("SELECT user_key FROM users WHERE user_id ='".$userid."'")->row();
            $user_key   =  $data->user_key;
            if($user_key == $unique_key){
...
if ( ! function_exists('_is_super_login'))
{
     function _is_super_login($thi)
    {
        $userid     = _get_current_user_id($thi);
        $usertype   = _get_current_user_type_id($thi);
        $is_access  = _get_user_type_access($thi,$usertype);
            ...
            if($user_key == $unique_key){
                if(isset($userid) && $userid!="" && isset($usertype)){
                    if($is_access == true){
                        ...
                        //echo "by"; exit;
                        $thi->load->view("superadmin/common/not_access");
                        return false;    
                    }
...
if(! function_exists('_get_current_user_key')){
    function _get_current_user_key($thi){
        return $thi->session->userdata("unique_key");
    }
}
...
if(! function_exists('_get_user_type_access')){
    function _get_user_type_access($thi,$user_type_id){
            ...
            $result     = $thi->db->query("select * from user_type_access where user_type_id = '".$user_type_id."'  and (method = '".$cur_method."' or method='*')"); //and class = '".$cur_class."'
            $row        = $result->row();
...
if(! function_exists('_get_user_redirect')){
    function _get_user_redirect($thi){
        if(_get_current_user_type_id($thi)==2){
