                        ...
                        <table border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="unit"><?=$this->config->item('currency').number_format($row['price'], 2, '.', '').'('.$row['qty_in_kg'].' '.$row['unit'].')'?></td>
                                    <td class="qty"><?=$row['qtys']?></td>
                                    <td class="total"><?=$this->config->item('currency')?><?=number_format($row['price']*$row['qtys'], 2, '.', '')?></td>
                                </tr>
                            <?php 
                                    $sn =$sn+1; 
                            ...
                            <tfoot>
								...
								<?php 
    								...
    								$grandtotal = $subtotal+@$delivery_charge-@$orderList['coupan_amount_use'];
    								
    							$paybleamount = $grandtotal-$orderList['wallet_amount']+$orderList['vat_amount'];
                                ...
                                <tr>
                                    ...
                                    <td><?=$this->config->item('currency').number_format($orderList['vat_amount'], 2, '.', '')?></td>
                                </tr>
                                <tr>
                                    <td colspan="2"></td>
                                    <td colspan="2"><?=$this->lang->line("GRAND TOTAL")?></td>
                                    <td><?=$this->config->item('currency')?><?=number_format(($grandtotal + $orderList['vat_amount']), 2, '.', '')?></td>
                                </tr>
                                <tr>
                                    <td colspan="2"></td>
                                    <td colspan="2"><?=$this->lang->line("Wallet Amount")?></td>
                                    <td><?="- ".$this->config->item('currency').number_format($orderList['wallet_amount'], 2, '.', '')?></td>
                                </tr>
                                <tr>
                                    <td colspan="2"></td>
                                    <td colspan="2"><?=$this->lang->line("Payble Amount")?></td>
                                    <td><?=$this->config->item('currency').number_format($paybleamount, 2, '.', '')?></td>
                                </tr>
                            </tfoot>
                        </table>
